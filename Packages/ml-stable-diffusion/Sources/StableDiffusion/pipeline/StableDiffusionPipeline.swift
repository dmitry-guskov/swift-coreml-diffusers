...
public struct StableDiffusionPipeline: StableDiffusionPipelineProtocol {
    ...
    func generateLatentSamples(configuration config: Configuration, scheduler: Scheduler) throws -> [MLShapedArray<Float32>] {
        ...
        var samples = (0..<config.imageCount).map { _ in
...
public struct PipelineProgress {
    ...
    public let initialLatentSamples: [MLShapedArray<Float32>]
...
extension StableDiffusionPipelineProtocol {
    ...
    func noiseSample(from data: Data?, shape: [Int]?, expectedShape: [Int]) -> MLShapedArray<Float32>? {
        guard let data, let shape else { return nil }
        guard shape == expectedShape else { return nil }
        let scalarCount = shape.reduce(1, *)
        let expectedByteCount = scalarCount * MemoryLayout<Float32>.size
        guard data.count == expectedByteCount else { return nil }
        let scalars: [Float32] = data.withUnsafeBytes { rawBuffer in
    ...
    func blendedNoise(_ previousNoise: MLShapedArray<Float32>, _ currentNoise: MLShapedArray<Float32>, amount: Float) -> MLShapedArray<Float32> {
        ...
        return MLShapedArray<Float32>(unsafeUninitializedShape: currentNoise.shape) { result, _ in
