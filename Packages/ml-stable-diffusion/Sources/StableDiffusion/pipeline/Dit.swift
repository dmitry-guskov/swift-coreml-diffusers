...
///   - stage 0:  inputs `latents`, `timestep`, `cap_feats` (+ optional LoRA inputs)  →  output `hidden_tokens`
///   - stage 1‒5: inputs `hidden_tokens`, `timestep`, `cap_feats` (+ optional LoRA inputs)  →  output `hidden_tokens`
///   - stage 6:  inputs `hidden_tokens`, `timestep`, `cap_feats` (+ optional LoRA inputs)  →  output final noise tensor
...
public struct Dit: ResourceManaging {
    ...
    func predictNoise(
        ...
        let results = try predictions(from: batch, loraInputProvider: loraInputProvider)
